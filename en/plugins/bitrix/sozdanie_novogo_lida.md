...
Для создания нового лида Bitrix перейдите по [ссылке](https://admin.corezoid.com/folder/conv/8316) и клонируйте шаблон процесса `"Create new lead"`.
...
При создания нового лида обязательным параметром является его название (title).

Значение других параметров передается в API аналогично:
fields[NAME] - имя нового лида
fields[CURRENCY_ID] - валюта и т.д.
...
**В случае ошибки**, заявка уйдет в узел эскалации и будет добавлен параметры
* `Error` - описание ошибки



>[Документация](https://dev.1c-bitrix.ru/rest_help/crm/leads/crm_lead_add.php) Bitrix для разработчиков.
