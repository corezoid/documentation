...
2. Перейдите на вкладку **Service accounts**
    ...
    2.4. Для начала добавьте имя для Вашей базы данных. 
   ...
   ![img](../../../en/plugins/firebase/img/add-values-to-firebase.png)
    ...
    2.5. Следующим элементом в **URL** рекомендуем добавить имя таблицы (коллекции) объектов, которые Вы будете сохранять, например, **clients**:

    ```
    https://test-xxx.firebaseio.com/testdb/clients
    ```
    
    2.6. Для хранения информации о клиенте создайте индекс с номером **1**. У каждого нового клиента в БД будет свой уникальный номер таблицы для хранения его имени и фамилии.
 
    2.7. Добавьте в индекс **1** параметры клиента **first name** и **last name**.
