...
![img](../ru/interface/img/releases/v4_1.png)
2. Реализована динамическая подстановка ключа для customize response parameters - имя ключа может быть зависимым от атрибутов заявки.<br/>
Примеры:<br/>
`{{test}}`<br/>
`{{test.test}}`<br/>
`{{test[0].key}}`<br/>
Ключ формируется от набора параметров, которые пришли в API Call узел. Т.е. система не будет брать параметры, которые получились после выполнения API. Если ключ не может быть построен, то он будет заменен на пустое значение.
3. Добавлена возможность отправки **RAW FORMAT** template из узла API Call.<br/>
Поддерживаются следующие форматы данных: string/number/array/object.<br/>
Пример отправки payload как массива (array):<br/>
